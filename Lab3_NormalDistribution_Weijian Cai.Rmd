...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    distribution?  
    Yes, I would say is nearly normal distribution, slightly right skewed.
...
3.  Make a normal probability plot of `sim_norm`.  Do all of the points fall on 
    ...
    data?  
    The real data is very similar to this, most of point fall on the line.
```{r}
qqnorm(sim_norm)
qqline(sim_norm)
```
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    The "fdmis$hgt" plot is similar to the simulated data plot. So we can say the female heights are nearly noral.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.    
    
    compare to other normal distribution plot, it looks different. So the female wights is not nearly normal distribution.
...
6.  Write out two probability questions that you would like to answer; one 
    ...
    1.What is the probability that a female's height is less than 158?
The results between the normal distribution and the empricial calculation differ by 0.0122.
```{r}
pnorm( q=158, mean = fhgtmean, sd = fhgtsd)
sum(fdims$hgt < 158) / length(fdims$hgt)

```

   2.What is the probability that a female's height is between 60 and 65 kg?
They differ by less than 3%.
* * *
```{r}
fwgtmean <- mean(fdims$wgt)
fwgtsd   <- sd(fdims$wgt)
pnorm(q=65,mean=fwgtmean,sd=fwgtsd) - pnorm(q=60,mean=fwgtmean,sd=fwgtsd)

sum(fdims$wgt < 65 & fdims$wgt >60)/ length(fdims$wgt)
```
...
-   Now let's consider some of the other variables in the body dimensions data 
    ...
    **d.** The histogram for female chest depth (`che.de`) belongs to normal 
    probability plot letter ___A_.

-   Note that normal probability plots C and D have a slight stepwise pattern.  
    Why do you think this is the case?  
    
    Both of them is nearly normal distribution, there are some small deviations from how a normal distribution.
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    from qq plot, it seems like right skewed.
```{r}
qqnorm(fdims$kne.di)
qqline(fdims$kne.di)


fdimean<-mean(fdims$kne.di)
fdisd<-sd(fdims$kne.di)
hist(fdims$kne.di, probability = TRUE)
x <- 5:30
y <- dnorm(x = x, mean = fdimean, sd = fdisd)
lines(x = x, y = y, col = "blue")
