...
1.  What type of plot would you use to display the relationship between `runs` 
    ...
    ```
    The plots  appear to be linear. I would be comfortable using a linear model.
    ```
```{r}
library(ggplot2)
ggplot(mlb11,aes(x=at_bats,y=runs)) + geom_point() + xlab("At Bats") + ylab("Runs")
```
...
```
The relationships appears to be linear, the direction is positive.
```
...
```{r}
homeruns_runs <- lm(runs ~ homeruns, data=mlb11)
summary(homeruns_runs )
```
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Yes. This histogram appears to be symmetric and unimodal.
...
8.  Based on the plot in (1), does the constant variability condition appear to 
    ...
    Yes, the residuals appears along the line, we can assume it is constant variance.
* * *
...
-   Choose another traditional variable from `mlb11` that you think might be a 
    ...
    I pick 'hits', it semms to be a linear relationship.
```{r}
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    for hits model, r squared is 0.6419, which is a little better than previous model.
```{r}
summary(m2)
summary(m1)
```
...
-   Now examine the three newer variables. These are the statistics used by the 
    ...
    The on base + slugging percentage is the best predictor of runs, which has the highest R2 value.
```{r}
...
-   Check the model diagnostics for the regression model with the variable you 
 ...
 histogram that shows a symmetric and unimodal distribution, which can tells it is linearity.
```{r}
hist(resid(m4))

plot(mlb11$new_obs, resid(m4))
abline(h = 0, lty = 3)
```
