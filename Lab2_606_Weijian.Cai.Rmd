...
####1 means one hit and then follow by one miss. There are one streak of 1. There are 4 streak of 0.
...
####Typical streak is 0. His longest streak is 2.
...
-   Describe the distribution of streak lengths. What is the typical streak 
    ...
    ####we use summary table and barplot, it is right skewed, the typical streak is 0.The longest streak is 4.
...
-   If you were to run the simulation of the independent shooter a second time, 
    ...
    ###I expect the streak distribution is similar, since we consider 133 times is a large sample set.
...
-   How does Kobe Bryant's distribution of streak lengths compare to the 
    ...
    ####we compare kobe streak with the independent streak, both looks similar, we can not say kobe fit the hot hand model.
    ```{r}
    kobe_streak <- calc_streak(kobe$basket)
    barplot(table(kobe_streak))
    barplot(table(sim_streak))
