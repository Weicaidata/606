...
```
This is observational study.I would rephase it as"Is there correlation between beauty and the differences in course evaluations?"
```
...
2.  Describe the distribution of `score`. Is the distribution skewed? What does 
    ...
    It is right skewed. It shows that sutdents are tend to give a high score. Yes, that was what I expected. Since most of the teachers were doing a job.
```{r}
hist(evals$score)
```
...
3.  Excluding `score`, select two other variables and describe their relationship 
    ...
    mosaic plot).  
    tenured teachers have the highest scores.
```{r}
...
4.  Replot the scatterplot, but this time use the function `jitter()` on the
    ...
    the initial scatterplot shows less points than the jitter plot.
```{r}
...
y=3.88034+0.06664×avgbeautyrating
```{r}
plot(jitter(evals$score) ~ evals$bty_avg)
...
the histogram of residuals is slightly left skewed. The Normal Probability Plot falls out the lines for upper quadriles.
```{r}
qqnorm(m_bty$residuals)
qqline(m_bty$residuals)
...
Yes, it is a signigicant predictor of 'score'.
...
```
the male has the higher score, since  ??^2 is a positive value.
```
...
11. Which variable would you expect to have the highest p-value in this model? 
    ...
    I would expect the class credits to have the highest p-value.
...
12. Check your suspicions from the previous exercise. Include the model output
    ...
    It is the second highest p value.

13. Interpret the coefficient associated with the ethnicity variable.  
```
When  other variable held constant, the score will increase by 0.1234929, when ethnicity is not a minority.
``` 
...
14. Drop the variable with the highest p-value and re-fit the model. Did the
    ...
    the p value of each variables doesn not change singificantly.
```{r}
drop_cls_prof <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
         + cls_students + cls_level + cls_credits + bty_avg 
         + pic_outfit + pic_color, data = evals)
summary(drop_cls_prof)
```
...
score^=3.907+0.164xethnicitynot_minority+0.203×gendermale???0.007×age+0.047×bty_avg
```{r}
best <- lm(score ~ ethnicity + gender +  age +  
            cls_credits + bty_avg 
...
17. The original paper describes how these data were gathered by taking a
    ...
    Yes, it doese have a impact on the linear regression.

18. Based on your final model, describe the characteristics of a professor and 
    course at University of Texas at Austin that would be associated with a high
    evaluation score.  
```
professor have the high score tend to be who is not minority male.
```
...
19. Would you be comfortable generalizing your conclusions to apply to professors
    ...
    Yes, I would like to share with others.
