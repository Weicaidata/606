...
2.  The title of the report is "Global Index of Religiosity and Atheism". To
    ...
    assumption?  
    ```
    1.samples are independent. 2. each category are more than 10.
    ```
...
```
Each row of Table6 correspond to each country. Each row of 'atheism' correspond to individual peroson.
```
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    yes, it agree with table 6.

```{r us-atheism, eval=TRUE}
us12 <- subset(atheism, nationality == "United States" & year == "2012")
head(us12)

us12tbl <- table(as.factor(us12$nationality),us12$response)
us12p <- prop.table(us12tbl)
us12p
```
...
```
the samples are independent. and the a success and failure rate are greater than 10.
```
...
inference(ita12$response, est = "proportion", type = "ci", method = "theoretical", 
      success = "atheist")

Austria12 <- subset(atheism, nationality == "Austria" & year == "2012")
inference(Austria12$response, est = "proportion", type = "ci", method = "theoretical", 
      success = "atheist")
...
```
The margin of error is largest when p = 0.5 and lowest when p =0 & p =1.
```
...
hist(p_hats, main = "n = 1040,p = 0.1", xlim = c(0, 0.18))
...
hist(p_hats2, main = "n = 400,p = 0.2", xlim = c(0, 0.18))
...
hist(p_hats3, main = "n = 1040,p = 0.2", xlim = c(0, 0.18))
...
hist(p_hats4, main = "n = 400,p = 0.2", xlim = c(0, 0.18))
```
...
```
Australia is 104, which it good to proceed.but the Expected Value for Ecuador is 8, which is less than 10, it might not good to proceed.
```
...
```
expect to detect a change about 1.95, or 2 countries.
```
