...
`length`. 
It is more than 30 samples size, and it is less than 10% of all the births, the sample are ramdomly selected, so the conditions for inference are satisfied.
```{r}
...
Ho: the average weight is same for smoking and non-smoking mothers.
Ha: the average is different between smoking and non-smoking mothers.
...
we can see that the p-value is larger than 0.05, which is the sinificant value, Therefore, we fail to reject the null hypothesis.  
...
I would say the cutoff age is 34, the mature mom is between 35 and 50, the younger mon is between 13 and 34.
...
a hypothesis test on whether or not the week  is dependent on the habit.  
...
we can see that the p-value is greater than the significance level. Therefore, we can not refect the null hypothesis.
